...
function ContactList({ user, logout, db, ...props }) {
  ...
  return (
    <div className="bg-slate-800 col-span-2 ">
      ...
      <div
        className="flex items-center justify-between text-gray-300 text-lg 
      px-3 py-2"
      >
      ...
      <div className="mt-4 overflow-y-scroll max-h-[32rem] pb-6">
        {ContactArray.filter((item) => item.uid !== user.uid).map((item) => (
          <button onClick={() => XYZ(item)} key={item.uid} className=" w-full">
            <ContactCard showName={item.displayName} showImg={item.photoURL} />
          </button>
        ))}
      </div>
    </div>
